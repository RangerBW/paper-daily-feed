...
- 了解具体研究领域后，基于网络搜索列出与该领域最相关的 Top 期刊，再询问用户希望订阅、补充或排除哪些期刊，并根据回答设置论文来源。借助期刊或出版社提供的 RSS、Crossref 等现有数据源，核对刊名、ISSN 和可用订阅源；最终给出关注期刊名由用户确认。
- 内置期刊主要面向地理科学。其他学科默认只订阅用户确认的期刊，不混入地理科学期刊；如果用户没有明确期刊，可以借助上述数据源协助其梳理候选项，但不要自行扩大订阅范围。
...
- [ ] **确认关注期刊：** 根据已确认的具体研究领域进行网络搜索，先向用户列出搜索结果中与该领域最相关的 Top 期刊，再询问其希望订阅、补充或排除哪些期刊。随后借助期刊或出版社提供的 RSS、Crossref 等现有数据源核对刊名、ISSN 和订阅源。获得用户确认后，将期刊写入 `interests.profile.favoriteJournals`。

    地理科学用户从 `data/journals.config.ts` 选择并写入 `feeds.catalogSelections`。非地理科学用户默认设置 `feeds.includeCatalog: false`，并将可用 RSS 写入 `feeds.customRss`。

    不要把空的 `catalogSelections` 当作“不选择内置期刊”，因为空数组表示选择全部。
